    ...
    handleEntityUpdateDom(entityInfo) {
      ...
      for (let ruleInfo of entityInfo.ruleInfos) {
        for (let svgElementId in ruleInfo.svgElementInfos) {
          ...
          } else if ( $(svgElementInfo.svgElement).is('g') && ruleInfo.rule.text_template ) {
            for ( let i = 0; i < $(svgElementInfo.svgElement).children().length; i++ ) {
              let elem = $($(svgElementInfo.svgElement).children()[i])
              if ( elem.is('text') ) {
                let self = this
                let updateGroupText = function(elem, entityId, ruleInfo, svgElementInfo) {
                  const svgElement = svgElementInfo.svgElement;
                  const state = self.hass.states[entityId] ? self.hass.states[entityId].state : undefined;

                  const text = ruleInfo.rule.text_template ? self.evaluate(ruleInfo.rule.text_template, entityId, svgElement) : state;

                  const tspan = $(elem).find('tspan');
                  if (tspan.length) {
                    $(tspan).text(text);
                  }
                }
                updateGroupText(elem, entityId, ruleInfo, svgElementInfo)
              }
            }
          }
